...
const DocScanner = ({ onBack, onNotify }) => {
  // State: Holds array of page objects { id, src, filter, contrast }
  const [pages, setPages] = useState([]);
  const [activeIndex, setActiveIndex] = useState(0);
  const [applyToAll, setApplyToAll] = useState(false);
  
  // Real-time Preview Canvas State
  ...
  // --- 🪄 THE MAGIC CANVAS ENGINE (Fast & Offline) ---
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (filterType === 'original') {
            ctx.filter = `contrast(${cVal})`;
        } else if (filterType === 'grayscale') {
            ctx.filter = `grayscale(100%) contrast(${cVal + 0.2})`;
        } else if (filterType === 'b&w') {
            ctx.filter = `grayscale(100%) contrast(${cVal + 1.5}) brightness(1.15)`;
        } else if (filterType === 'magic') {
            ctx.filter = `contrast(${cVal + 0.4}) brightness(1.15) saturate(1.8)`;
        }
  ...
  useEffect(() => {
    if (pages.length > 0 && pages[activeIndex] && !isCropMode) {
        ...
        applyMagicFilter(activePage.src, activePage.filter, activePage.contrast)
            .then(setPreviewData);
    } else {
        setPreviewData(null);
    }
  ...
  const handleFiles = (e) => {
    ...
    if(onNotify) onNotify(null, true); // Haptic on upload
  };
  ...
  // --- ✂️ PROFESSIONAL AUTO-FIT CROP LOGIC ---
  const handleCropConfirm = () => {
      const cropper = cropperRef.current?.cropper;
      if (cropper) {
          // Extra high-quality export so text stays sharp
          ...
          setPages(prev => prev.map((page, idx) => {
              if (idx === activeIndex) {
                  return { ...page, src: croppedDataUrl }; // Update with cropped image
              }
              return page;
          }));
          
          setIsCropMode(false);
          if(onNotify) onNotify("Page Perfectly Cropped! ✂️", true);
      }
  };
  ...
  const handleApplyToAllChange = (e) => {
      ...
      if (isChecked && pages.length > 0) {
          const currentFilter = pages[activeIndex].filter;
          const currentContrast = pages[activeIndex].contrast;
          setPages(prev => prev.map(page => ({
              ...page, filter: currentFilter, contrast: currentContrast
          })));
          if(onNotify) onNotify(null, true); 
      }
  };

  // --- 📄 PDF GENERATION & AD LOGIC ---
  const generateAndSavePDF = async () => {
      ...
      const executeSave = async () => {
          ...
          if(onNotify) onNotify("PDF Saved Successfully! 📄✅", false, finalPdfName, "Scanned PDF", pdfBlob);
      };
  ...
  const S = {
    ...
    thumbnailWrap: (isActive) => ({ position: 'relative', minWidth: '60px', height: '75px', borderRadius: '6px', border: isActive ? '3px solid #3b82f6' : '1px solid #334155', overflow: 'hidden', cursor: 'grab', opacity: isActive ? 1 : 0.6, transition: '0.2s' }),
    ...
    addBtnSmall: { minWidth: '60px', height: '75px', borderRadius: '6px', border: '2px dashed #475569', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#1e293b', color: '#94a3b8', cursor: 'pointer' },
    ...
    filterBtn: (isActive) => ({ flex: 1, padding: '10px 12px', borderRadius: '8px', background: isActive ? '#3b82f6' : '#334155', color: isActive ? 'white' : '#94a3b8', border: 'none', cursor: 'pointer', fontSize:'12px', fontWeight:'bold', whiteSpace: 'nowrap' }),
    sliderBox: { display: 'flex', flexDirection: 'column', gap: '8px' },
    sliderInput: { width: '100%', accentColor: '#3b82f6', height: '4px', cursor:'pointer' },
    pdfButton: { background: 'linear-gradient(90deg, #ec4899, #8b5cf6)', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: 'bold', fontSize: '16px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', cursor: 'pointer', width: '100%', boxShadow: '0 4px 15px rgba(236, 72, 153, 0.3)' }
  };
    ...
    <div style={S.wrapper}>
         ...
         <div style={{position:'absolute', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:60}}>
             <div style={{background:'#1e293b', padding:'24px', borderRadius:'16px', width:'85%', maxWidth:'350px', border:'1px solid #334155'}}>
                 ...
                 <div style={{display:'flex', gap:'10px'}}>
                     <button onClick={() => setShowRenameDialog(false)} style={{flex:1, padding:'12px', background:'#334155', color:'white', border:'none', borderRadius:'8px', fontWeight:'bold'}}>Cancel</button>
                     <button onClick={generateAndSavePDF} style={{flex:1, padding:'12px', background:'#3b82f6', color:'white', border:'none', borderRadius:'8px', fontWeight:'bold'}}>Save PDF</button>
                 </div>
             </div>
         </div>
      )}
      ...
      {showAddMenu && (
        <div style={{position:'absolute', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'flex-end', zIndex:60}} onClick={() => setShowAddMenu(false)}>
            <div style={{background:'#1e293b', width:'100%', borderTopLeftRadius:'24px', borderTopRightRadius:'24px', padding:'25px 20px', display:'flex', flexDirection:'column', gap:'12px', boxShadow:'0 -10px 40px rgba(0,0,0,0.5)'}} onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={() => setShowAddMenu(false)} style={{padding:'15px', background:'transparent', border:'none', color:'#ef4444', fontWeight:'bold', fontSize:'16px', marginTop:'5px', cursor:'pointer'}}>
                    Cancel
                </button>
            </div>
        </div>
      ...
      <div style={S.header}>
        <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
            <button onClick={() => isCropMode ? setIsCropMode(false) : onBack()} style={{background:'none', border:'none', color:'white', cursor:'pointer'}}><Icons.Back/></button>
            <span style={{fontWeight:'bold', fontSize:'16px'}}>{isCropMode ? "Crop Page" : "Magic Scanner"}</span>
        </div>
        
        {/* Toggle between Premium button and Crop Done button */}
        {isCropMode ? (
            <button onClick={handleCropConfirm} style={{padding: '6px 16px', borderRadius:'8px', border:'none', background: '#10b981', color: 'white', fontWeight: 'bold', fontSize: '14px', display:'flex', alignItems:'center', gap:'5px', cursor:'pointer'}}>
                Done <Icons.Check/>
            </button>
        ) : (
            <button onClick={() => setIsPremium(!isPremium)} style={{padding: '6px 10px', borderRadius:'20px', border:'none', background: isPremium ? '#f59e0b' : '#334155', color: isPremium ? '#fff' : '#94a3b8', fontWeight: 'bold', fontSize: '11px', display:'flex', alignItems:'center', gap:'5px', cursor:'pointer'}}>
         ...
         {pages.length === 0 ? (
            <div style={{display:'flex', flexDirection:'column', gap:'20px', zIndex:5, alignItems:'center'}}>
                ...
                <div style={{display:'flex', gap:'15px', marginTop:'10px'}}>
                    <button onClick={() => cameraInputRef.current.click()} style={{background:'#3b82f6', color:'white', padding:'16px 24px', borderRadius:'12px', border:'none', fontWeight:'bold', display:'flex', alignItems:'center', gap:'10px', fontSize:'16px', cursor:'pointer', boxShadow:'0 4px 15px rgba(59, 130, 246, 0.4)'}}>
                        <Icons.Camera /> Camera
                    </button>
                    <button onClick={() => fileInputRef.current.click()} style={{background:'#334155', color:'white', padding:'16px 24px', borderRadius:'12px', border:'none', fontWeight:'bold', display:'flex', alignItems:'center', gap:'10px', fontSize:'16px', cursor:'pointer'}}>
             ...
             <>
                 {isCropMode ? (
                     ...
                     <>
                         ...
                         {/* Crop Button */}
                         <button onClick={() => setIsCropMode(true)} style={{position:'absolute', top:'15px', left:'15px', background:'rgba(59, 130, 246, 0.2)', color:'#3b82f6', border:'1px solid rgba(59, 130, 246, 0.5)', padding:'8px', borderRadius:'8px', zIndex:10, cursor:'pointer'}}>
                             <Icons.Crop />
                         </button>

                         {/* Delete Button */}
                         <button onClick={() => removePage(activeIndex)} style={{position:'absolute', top:'15px', right:'15px', background:'rgba(239, 68, 68, 0.2)', color:'#ef4444', border:'1px solid rgba(239, 68, 68, 0.5)', padding:'8px', borderRadius:'8px', zIndex:10, cursor:'pointer'}}>
                             <Icons.Trash />
                         </button>
                     </>
                 )}
      ...
      {/* TRAY & CONTROLS (Hidden during crop mode for better focus) */}
